...
Shell::Shell(ShellType type, Vector2 position) : Enemy(position) {
	...
	if (type == GREEN_SHELL) {
		inShell = RESOURCE_MANAGER.getAnimation("enemies1.txt/green_KoopaTroopa_shell")->clone();
		outShell = RESOURCE_MANAGER.getAnimation("enemies1.txt/green_KoopaTroopa_outShell")->clone();
		shellSpin = RESOURCE_MANAGER.getAnimation("enemies1.txt/green_paraKoopaTroopa_spin")->clone();
	}
	else if (type == RED_SHELL) {
		inShell = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_KoopaTroopa_shell")->clone();
		outShell = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_KoopaTroopa_outShell")->clone();
		shellSpin = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_paraKoopaTroopa_spin")->clone();
	}
