...
GoomBa::GoomBa(GoomBaType type, Vector2 position, Orientation orientation) : Enemy(position) {
	...
	if (type == BROWN_GoomBa) {
		walkAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_goomba")->clone();
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_goomba_dead");
	}
	else if (type == RED_GoomBa) {
		walkAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_goomba")->clone();
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_goomba_dead");
	}
	else if (type == BROWNPARA_GoomBa) {
		walkAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_paragoomba")->clone();
		jumpAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_paragoomba_jump")->clone();
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_goomba_dead");
		this->canJump = true;
	}
	else if (type == REDPARA_GoomBa) {
		walkAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_paragoomba")->clone();
		jumpAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_paragoomba_jump")->clone();
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_goomba_dead");
		this->canJump = true;
	}
...
void GoomBa::attacked(Orientation direction) {
	...
	if (type == BROWN_GoomBa || type == BROWNPARA_GoomBa) {
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/brown_goomba")->clone();
	}
	else if (type == RED_GoomBa || type == REDPARA_GoomBa) {
		deadAnimation = RESOURCE_MANAGER.getAnimation("enemies1.txt/red_goomba")->clone();
	}
