...
Star::Star(StarType type, Vector2 position, Orientation orientation) : Item(POINT) {
	...
	if (type == YELLOW_STAR) {
		starAnimation = RESOURCE_MANAGER.getAnimation("miscellaneous.txt/yellow_star_item")->clone();
	}
	else if (type == BLUE_STAR) {
		starAnimation = RESOURCE_MANAGER.getAnimation("miscellaneous.txt/blue_star_item")->clone();
	}
