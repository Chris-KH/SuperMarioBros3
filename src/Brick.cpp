...
Brick::Brick(Vector2 pos, Vector2 size) : BaseBlock(pos, size) {
    sprite = RESOURCE_MANAGER.getAnimation("tiles1.txt/gold_brick_block")->clone();
    setAnimation(sprite);
}
...
void Brick::breakBrick() {
    killEntity();
    Effect* breakEffect = new Effect(RESOURCE_MANAGER.getAnimation("tiles1.txt/gold_brick_block_broken")->clone(), getPosition(), true, 0.f);
